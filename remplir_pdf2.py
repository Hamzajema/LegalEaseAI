...
CONTENU DU PDF (avec les champs vides marqués par des pointillés « … », des parenthèses « (…) », des crochets « [ ] », des tirets « --- » ou des lignes « _____ », ou tout autre symbole similaire):
...
def ask_user_for_missing_fields(missing_fields: list, filled_fields: list) -> Dict[str, Any]:
    """Demande à l'utilisateur de fournir les informations manquantes"""
    
    print("\n" + "="*60)
    print("⚠️  INFORMATIONS MANQUANTES DÉTECTÉES")
    print("="*60)
    print(f"\nL'IA a identifié {len(filled_fields)} champs pouvant être remplis.")
    print(f"Mais {len(missing_fields)} champs nécessitent des informations supplémentaires.\n")
    
    additional_data = {}
    
    for i, field in enumerate(missing_fields, 1):
        print(f"\n[{i}/{len(missing_fields)}] Champ manquant: {field}")
        user_input = input(f"    Entrez la valeur pour '{field}' (ou appuyez sur Entrée pour ignorer): ").strip()
        
        if user_input:
            additional_data[field] = user_input
            print(f"    ✓ Enregistré: {user_input}")
        else:
            print(f"    ⊘ Ignoré")
...
def complete_fields_with_user_input(filled_fields: list, missing_fields: list, user_input: Dict[str, Any], page_number: int = 1) -> list:
    """Complète les champs avec les données fournies par l'utilisateur"""
    
    completed_fields = filled_fields.copy()
...
def process_pdf_with_ai(
    ...
    Args:
        pdf_path: Chemin du PDF à remplir
        user_data: Données utilisateur disponibles
        output_path: Chemin du PDF de sortie
        interactive: Si True, demande les infos manquantes à l'utilisateur
    """
    ...
    if missing_fields and interactive:
        # Demander les informations manquantes à l'utilisateur
        additional_data = ask_user_for_missing_fields(missing_fields, filled_fields)
        
        if additional_data:
            # Compléter les champs avec les nouvelles données
            filled_fields = complete_fields_with_user_input(
                filled_fields, 
                missing_fields, 
                additional_data,
                page_number=1  # Peut être ajusté selon le contexte
            )
            filled_data["filled_fields"] = filled_fields
            filled_data["missing_fields"] = [f for f in missing_fields if f not in additional_data]
            
            print(f"\n✅ {len(additional_data)} champs supplémentaires ajoutés !")
    
    # Afficher un résumé avant création
    print("\n" + "="*60)
    print("📋 RÉSUMÉ AVANT CRÉATION DU PDF")
    print("="*60)
    print(f"✓ Champs à remplir: {len(filled_fields)}")
    print(f"⚠️ Champs restants vides: {len(filled_data.get('missing_fields', []))}")
    
    if filled_data.get('missing_fields'):
        print("\nChamps qui resteront vides:")
        for field in filled_data['missing_fields']:
            print(f"  • {field}")
    
    # Demander confirmation
    if interactive:
        print("\n" + "="*60)
        confirm = input("Voulez-vous créer le PDF maintenant ? (o/n): ").strip().lower()
        if confirm not in ['o', 'oui', 'y', 'yes']:
            print("❌ Création du PDF annulée.")
            return {
                "success": False,
                "output_path": None,
